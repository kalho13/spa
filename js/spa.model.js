...
spa.model = (function(){
    ...
    Its public methods include:
      ...
      * get_db() - return the TaffyDB database of all the person  objects - including the current
        user - presorted.
 
      * get_by_cid( <client_id> ) - return a person object with provided unique id.
 
      * login( <user_name> ) - login as the user with the provided user name. The current user object
        is changed to reflect the new identity. Successful completion of login publishes a 'spa-login'
        global custom event.
 
      * logout()- revert the current user object to anonymous. This method publishes a 'spa-logout'
        global custom event.
 
    jQuery global custom events published by the object include:
 
      * spa-login - This is published when a user login process completes. The updated user object is
       provided as data.
 
      * spa-logout - This is published when a logout completes. The former user object is provided as data.
    ...
    */
        ...
        completeLogin = function(user_list){
            ...
            stateMap.user.css_map[user_map._id] = stateMap.user;
        ...
        people = (function(){
            ...
            login = function(name){
                ...
                stateMap.user = makePerson({
                    cid : makeCid(),
                    css_map : {top:25, left:25, 'backgropund-color':'#8f8'},
                    name : name
                });
            ...
            logout = function(){
                var user = stateMap.user;
                //when we add chat we should leave the chat room here
...
/*The chat object API
    ...
    chat messaging. Its public methods include:
      ...
      * join() - joins the chat room. This routine sets up the chat protocol with the backend
       including publishers for 'spa-listchange' and 'spa-updatechat' global custom events.
        If the current user is anonymous, join() aborts and returns false.
 
      * get_chatee() - return the person object with whom the user is chatting. If there is no chatee,
       null is returned.
 
      * set_chatee( <person_id> ) - set the chatee to the person identified by person_id.
      If the person_id does not exist in the people list, the chatee is set to null. If the person
       requested is already the chatee, it returns false. It publishes a 'spa-setchatee' global custom event.
 
      * send_msg( <msg_text> ) - send a message to the chatee. It publishes a 'spa-updatechat' global custom
       event. If the user is anonymous or the chatee is null, it aborts and returns false.
 
      * update_avatar( <update_avtr_map> ) - send the update_avtr_map to the backend. This results in an
       'spa-listchange' event which publishes the updated people list and avatar information
        (the css_map in the person objects). The update_avtr_map must have the form 
    ...
    jQuery global custom events published by the object include:
      ...
      * spa-listchange - This is published when the list of online people changes in length
       (i.e. when a person joins or leaves a chat) or when their contents change (i.e. when a 
       person's avatar details change). A subscriber to this event should get the people_db from
        the people model for the updated data.
...
*/
    ...
    chat = (function(){
        ...
        _update_list = function(arg_list){
            ...
            //If chatee is no longer online, we must unset the chatee which triggers
        ...
        send_msg = function(msg_text){
            ...
            //we published updatechat so we can now show our outgoing message
