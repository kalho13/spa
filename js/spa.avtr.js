...
//-----------------------BEGIN UTILITY METHODS --------------------------------
    getRandRgb = function(){
        ...
        //return the randomly generated rgb value.  Generates (3) 3 digit numbers and adds them to the list
        return 'rgb('+rgb_list.join(',') + ')'; //returns the list seperated by commas
    };
...
updateAvatar = function($target){
    ...
    //passes in a map to update the avatar.  The map contains a sub-map css_map
...
/*
    Create the onHeldNav event handler.  This is triggered when the user releases an
    avatar after a drag.  The handler returns the dragged avatar back to its original color. 
    It then invokes the updateAvatar method to read the avatar details and invoke the
...
/*
    Create the onSetChatee event handler.  This is invoked when the Model publishes
    an spa-setchatee event.  In this module, we set the outline of the chatee avatar to green.
*/
...
onListchange = function(event){
    var
        ...
        //iterate through all of the people and see if they are anon, a user or the chatee
        people_db().each(function(person, idx){
            ...
            //add a box for each person in the list
            $box = $('<div/>')
...
// is updated with provided arguments. No other actions are taken.
// Returns : none
