...
spa.chat = (function () {
  ...
  setPxSizes = function () {
   ...
   // px_per_em = getEmSize( jqueryMap.$slider.get(0) );
    ...
    //if the window open height is less than the defined min then use the nin value
   opened_height_em
  ...
  */
  ...
  setSliderPosition = function ( position_type, callback ) {
    ...
    jqueryMap.$slider.animate(  //the $slider is the subset of chat html defined earlier
      { height : height_px },
      animate_time,
      function () {
        jqueryMap.$toggle.prop( 'title', slider_title );
        jqueryMap.$toggle.text( toggle_text );
        stateMap.position_type = position_type;  //this is where the url is change, when then calls onHashChange  ??
  ...
  //End private DOM methods to manage chat
  ...
  //
  configModule = function ( input_map ) {   //the input_map is generated with the argument in spa.shell
    spa.util.setConfigMap({
      input_map    : input_map, //pass the input_map
      settable_map : configMap.settable_map, //the settable_map portion of configMap.  It has to be in the settable list to be updated in the config
      config_map   : configMap //and the entire configMap
    });
    return true;
  };
...
removeSlider = function(){
  ...
  //remove DOM container; this removes event bindings as well
  if(jqueryMap.$slider){
  ...
  // return public methods  what exactly am I returning here and how is it being used?
  return {
